...
// lookupHost is a variable to allow mocking in tests
var lookupHost = utils.LookupHostGlobal
...
func NewTunnelService(repo repository.TunnelRepository, caddyService CaddyService, cfg *config.Config) interfaces.TunnelService {
...
func (s *tunnelService) isReservedDomain(domain string) bool {
...
func (s *tunnelService) CreateTunnel(ctx context.Context, userID uint32, domain string, targetPort int) (*ent.Tunnel, error) {
	...
	if s.isReservedDomain(domain) {
		...
		return nil, fmt.Errorf("%w: domain '%s' is reserved for system use and cannot be used for tunnels", ErrValidation, domain)
	}
	...
	isEnabled := true
	dnsPropagationStatus := tunnel.DNSPropagationStatusVerified
	...
	if utils.IsAutoGeneratedDomain(domain) {
		...
		for _, tunnel := range tunnels {
			if utils.IsAutoGeneratedDomain(tunnel.Domain) {
				return nil, fmt.Errorf("%w: user already has an auto-generated subdomain tunnel", ErrConflict)
			}
		}
		...
		if domain != expectedDomain {
			...
			return nil, fmt.Errorf("%w: invalid auto-generated domain for this user", ErrValidation)
		}
		...
		if s.config.ServerIP != "" {
			...
			if err == nil {
				...
				logger.Warn("Failed to lookup DNS for domain %s: %v", domain, err)
			}

			if !dnsVerified {
				logger.Warn("Domain %s does not point to server IP %s. Creating tunnel in DISABLED state.", domain, s.config.ServerIP)
				isEnabled = false
				dnsPropagationStatus = tunnel.DNSPropagationStatusPendingDNS
			}
		}
...
func (s *tunnelService) UpdateTunnel(ctx context.Context, userID uint32, tunnelID uint32, updates interface{}) (*ent.Tunnel, error) {
	...
	if u, ok := updates.(*repository.TunnelUpdate); ok {
		...
		// If enabling the tunnel, verify DNS for custom domains
		if u.IsEnabled != nil && *u.IsEnabled && !currentTunnel.IsEnabled {
			if !utils.IsAutoGeneratedDomain(currentTunnel.Domain) && s.config.ServerIP != "" {
				ips, err := lookupHost(currentTunnel.Domain)
				dnsVerified := false
				if err == nil {
					for _, ip := range ips {
						if ip == s.config.ServerIP {
					...
					logger.Warn("Failed to lookup DNS for domain %s: %v", currentTunnel.Domain, err)
				}

				if !dnsVerified {
					return nil, fmt.Errorf("%w: domain '%s' does not point to our server IP (%s). Please configure your DNS records first", ErrValidation, currentTunnel.Domain, s.config.ServerIP)
				}

				// DNS verified, set status to verified
				verifiedStatus := tunnel.DNSPropagationStatusVerified
				u.DnsPropagationStatus = &verifiedStatus
			}
		}
	}
