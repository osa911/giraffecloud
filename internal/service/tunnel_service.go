...
func NewTunnelService(repo repository.TunnelRepository, caddyService CaddyService, serverIP string) interfaces.TunnelService {
...
func (s *tunnelService) CreateTunnel(ctx context.Context, userID uint32, domain string, targetPort int) (*ent.Tunnel, error) {
	...
	if utils.IsAutoGeneratedDomain(domain) {
		...
		if s.serverIP != "" {
			...
			if !dnsVerified {
				logger.Warn("Domain %s does not point to server IP %s. Creating tunnel in DISABLED state.", domain, s.serverIP)
				isEnabled = false
...
func (s *tunnelService) UpdateTunnel(ctx context.Context, userID uint32, tunnelID uint32, updates interface{}) (*ent.Tunnel, error) {
	...
	if u, ok := updates.(*repository.TunnelUpdate); ok {
		...
		if u.IsEnabled != nil && *u.IsEnabled && !currentTunnel.IsEnabled {
			if !utils.IsAutoGeneratedDomain(currentTunnel.Domain) && s.serverIP != "" {
				...
				if !dnsVerified {
				if !dnsVerified {
					return nil, fmt.Errorf("%w: domain '%s' does not point to our server IP (%s). Please configure your DNS records first", ErrValidation, currentTunnel.Domain, s.serverIP)
				}
				}
